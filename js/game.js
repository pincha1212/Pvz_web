...
function gameLoop(timestamp){const deltaTime=Math.min(timestamp-GameState.lastTime,100);GameState.lastTime=timestamp;if(GameState.status==='PLAYING'){if(!GameState.gameOver&&!GameState.victory){if(GameState.currentMapDef.environment==='fog'){GameState.fogOffsetTimer+=deltaTime*.001;}if(GameState.currentMapDef.environment==='day'){GameState.skyResourceTimer+=deltaTime;if(GameState.skyResourceTimer>=9500){GameState.skyResourceTimer=0;GameState.entities.push(new ResourceDrop(Math.random()*(CANVAS_WIDTH-120)+60,0,true));}}WaveManager.update(deltaTime);for(const ent of GameState.entities)ent.update(deltaTime);for(const p of GameState.particles)p.update(deltaTime);for(const t of GameState.floatingTexts)t.update(deltaTime);GameState.entities=GameState.entities.filter(e=>!e.markedForDeletion);GameState.particles=GameState.particles.filter(p=>!p.markedForDeletion);GameState.floatingTexts=GameState.floatingTexts.filter(t=>!t.markedForDeletion);}ctx.clearRect(0,0,canvas.width,canvas.height);Renderer.drawBoard(ctx);Renderer.drawEntities(ctx);Renderer.drawFog(ctx);for(const p of GameState.particles)p.draw(ctx);for(const t of GameState.floatingTexts)t.draw(ctx);}requestAnimationFrame(gameLoop);}
